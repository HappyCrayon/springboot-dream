...
/**
 * 这里没有使用拦截 {@link org.apache.ibatis.executor.Executor 主要是因为PageHelp处理的时候，直接调用Executor的方法进行处理，没有调用invocation.proceed()
 * 下一个拦截器处理，直接处理SQL 的修改，因此，将这个拦截设置到最后的查询阶段去处理哦}
 *
...
public class MybatisSqlPrintInterceptor implements Interceptor, Ordered {
    ...
    private Logger log = LoggerFactory.getLogger(MybatisSqlPrintInterceptor.class);

    //这里为什么不生效
    @Value("${mybatis.sql.enable:true}")
    private boolean isShowSql = true;

    @Value("${mybaits.sql.displayParam:false}")
    private boolean isDisplayParamName;
        ...
        if (parameterMappings != null) {
            for (ParameterMapping parameterMapping : parameterMappings) {
                if (parameterMapping.getMode() != ParameterMode.OUT) {
                    ...
                    if (isDisplayParamName && !propertyName.contains("frch_criterion")) {
